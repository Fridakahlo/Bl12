...
if(!defined('VC_PARALLAX_BG_VERSION')){
	define('VC_PARALLAX_BG_VERSION', '1.5.1.1');
}
				...
				elseif($bg_type == "image"){
					...
					if($parallax_style == 'vcpb-fs-jquery' || $parallax_style=="vcpb-mlvp-jquery"){
						if($parallax_style == 'vcpb-fs-jquery')
							wp_enqueue_script('jquery.shake',plugins_url('assets/js/jparallax.js',__FILE__),array('jquery','jquery.video_bg'),VC_PARALLAX_BG_VERSION);
						
						if($parallax_style=="vcpb-mlvp-jquery")
							wp_enqueue_script('jquery.vhparallax',plugins_url('assets/js/jquery.vhparallax.js',__FILE__),array('jquery','jquery.video_bg'),VC_PARALLAX_BG_VERSION);
					...
					if($enable_controls == 'display_control')
						wp_enqueue_style('ultimate-vidcons',plugins_url('assets/fonts/vidcons.css',__FILE__),VC_PARALLAX_BG_VERSION);
				}
				elseif ($bg_type=='u_iframe') {
					//wp_enqueue_script('jquery.tublar',plugins_url('../assets/js/tubular.js',__FILE__));
					wp_enqueue_script('jquery.ytplayer',plugins_url('assets/js/jquery.mb.YTPlayer.js',__FILE__),VC_PARALLAX_BG_VERSION);
					$v_opts = explode(",",$video_opts);
					$v_img = wp_get_attachment_image_src($video_poster,'full');
					if(is_array($v_opts)){
						foreach($v_opts as $opt){
...
/*	Auto Update - Parallax & Video Backgrounds for Visual Composer 
 *-------------------------------------------------------------------*/
